...
VENTILATION_MAP = {
    'レベルA（特に換気のない部屋）': 4,
    'レベルB（全体換気）': 3,
    'レベルC（工業的な全体換気、屋外作業）': 1,
    '換気レベルD（外付け式局所排気装置）': 0.1,
    'レベルD（外付け式局所排気装置）': 0.1, # Alias
    'レベルE（囲い式局所排気装置）': 0.01,
    'レベルF（密閉容器内での取扱い）': 0.001
}
...
VARIATION_MAP = {
    '変動が大きい': 6,
    'ばく露濃度の変動が大きい作業': 6, # Alias from user example
    '変動が小さい': 4
}
...
SKIN_AREA_MAP = {
    '大きなコインサイズ': 10,
    '片手の手のひら': 240,
    '片手の手のひら付着': 240, # Alias from user example
    '両手の手のひら': 480,
    '両手全体': 960,
    '両手及び手首': 1500,
    '両手の肘から下全体': 1980
...
GLOVE_MAP = {
    '着用していない / 情報のない手袋': 1,
    '耐透過性・耐浸透性の手袋を着用': 0.2,
    '耐透過性・耐浸透性の手袋の着用している': 0.2 # Alias from user example
}
